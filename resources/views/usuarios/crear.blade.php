...
@section('content')
	...
	<div class="row">
		...
		<br><br>
			<form   method="POST" name="formulario" id="formulario" action="{{asset('usuarios/crear')}}">
				{{ csrf_field()}}
				...
				<div class="row">
						...
						<div class="col-sm-6">
							...
							<div class="input-group">
									...
									<select required="" name="identificacion" type="text" class="form-control" id="idni">
								          <option value="idni" >DNI </option>
			        					  <option value="iruc" >RUC </option>

			        </select>

							    <input class="form-control" type="text" name ="dni" id="DNI" placeholder="Ingrese identificación" required="true">
							</div>
							<div id="dniRepe"></div>
							<p id="noingreso"></p>
						</div>
...
<script>
   ...
   $("#formulario").submit(function(evento){
	...
	var indice = document.formulario.identificacion.value; 
		...
		if (indice == "iruc") {
			if( !(/^\d{11}$/.test(DNI)) ) {
			    texto ="Ingrese un RUC correcto. Verifique el número de digitos(11)";
			    alertify.set('notifier','position', 'top-right');
	     		 var msg = alertify.error('Default message');
				 msg.delay(10).setContent(texto);
				$("#porcentaje").html(" ");
